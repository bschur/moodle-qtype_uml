...
function initToolBoxClasses(graphToolBox: Graph) {
...
function initPaper(el: HTMLElement, graphToolBox: Graph, isInteractive: boolean) {
    return new Paper({
        el: el,
        model: graphToolBox,
        width: '100%',
...
export class UmlEditorComponent implements AfterViewInit {
    ...
    private setupJointJs() {
        ...
        /* TODO check code
        const graphEditor = new Graph();
        const graphToolBox = new Graph();
        ...
        /* paperToolbox.on('element:pointerup', (cellView, evt, x, y) => {
          const clickedClass = cellView // TODO check cellView.model.clone();
